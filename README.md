...
Pre-built binaries are available on the [Releases](https://github.com/openchamber/openchamber/releases/latest) page:

| Platform | Format | Notes |
|----------|--------|-------|
| **Linux x86_64** | AppImage | Requires FUSE or `APPIMAGE_EXTRACT_AND_RUN=1` |
| **Linux ARM64** | AppImage | Raspberry Pi 4+, Apple Silicon VMs |
| **Windows x64** | NSIS installer / portable zip | Windows 10+ |
| **macOS** | DMG (ARM64 + Intel) | macOS 13+ |
| **Android** | APK | Android 8+ |
| **iOS** | IPA (via TestFlight) | Requires Apple Developer account |

## Quick start

### Desktop — macOS, Windows, and Linux

Download the latest release from [Releases](https://github.com/openchamber/openchamber/releases/latest). Desktop bundles the matching OpenCode CLI, so no separate OpenCode installation is required.
...
# Without FUSE (containers, remote servers, WSL)
APPIMAGE_EXTRACT_AND_RUN=1 ./OpenChamber-*.AppImage
```

<details>
<summary>Alternative: extract and run manually</summary>

```bash
./OpenChamber-*.AppImage --appimage-extract
cd squashfs-root
./openchamber --no-sandbox
```
...
Run the NSIS installer, or extract the portable zip and launch `OpenChamber.exe`.

#### macOS

Open the DMG and drag OpenChamber to Applications.
...
Output: `packages/electron/dist/OpenChamber-*-linux-x86_64.AppImage`

### Windows portable (cross-compile from Linux)
...
Output: `packages/electron/dist/OpenChamber-*-win-x64.exe`
